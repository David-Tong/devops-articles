...
PR 合并后，代码变更正式成为主干的一部分。此时需要一条流水线将代码转化为可部署的制品，完成从单元测试到集成测试、再到预发环境的完整验证链路。
...
主干上随时都有新的构建产物，但不是每个构建都值得对外发布。发布流水线解决的是：**当团队决定发布某个版本时，如何规范、可追溯地完成这个动作**。
